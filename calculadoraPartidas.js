...
function calculaRank(vitorias, derrotas){
    ...
    console.log("O Herói tem de Saldo de "+ resultado + " e está no nível de " + rank);
}

function reiniciar(){
    rl.question('Deseja sair ou calcular novamente? ',(continuaPrograma)=>{
        if (continuaPrograma.toLowerCase() === "sair"){
            console.log("Programa encerrado.");
            rl.close();
            return;
        } else if (continuaPrograma.toLowerCase() === "calcular novamente"){
            iniciarCalculadoraRanque();
            return;
        } else{
            console.log("Entrada inválida. Digite 'sair' ou ' calcular novamente'");
            reiniciar(); 
            return;
        }
    })
